...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Sucr related functionality
...
/*
    ...
    allow endless transaction that would bloat Sucr and make it unusable. To
    stop these kinds of attacks 1 in 10 successful transactions are charged. This
    adds up to a cost of 0.001SUC per transaction on average.
*/
...
{
    if(fLiteMode) return; // disable all Sucr specific functionality
