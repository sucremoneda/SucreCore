...
const char * const BITCOIN_CONF_FILENAME = "sucr.conf";
const char * const BITCOIN_PID_FILENAME = "sucrd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sucr" is a composite category enabling all Sucr-related debug output
            if(ptrCategory->count(string("sucr"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SucrCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SucrCore
    // Mac: ~/Library/Application Support/SucrCore
    // Unix: ~/.sucrcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SucrCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SucrCore";
#else
    // Unix
    return pathRet / ".sucrcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sucr.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sucr.conf
