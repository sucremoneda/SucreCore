...
- Darksend tested with 2.5 SUC successfully and I've set that as the minimum
...
- Fixed the endless splitting issue causes by splitting 1000SUC and not making a DS compatible input
...
- Fixed issue with denominating small amounts of SUC in large wallet
  (http://jira.darkcoin.qa/browse/SUC-46)
- Made splitting up initial inputs much more efficient. Now when splitting up,
  it will use powers of two from 4096 SUC in reverse to get the best possible
...
- Fixed issue with completed amount jumping around
  (http://jira.darkcoin.qa/browse/SUC-46)
- Made messages less threatening (http://jira.darkcoin.qa/browse/SUC-60)
...
- Darksend limited to 5000SUC per wallet.dat. Client will warn about this the
  first time it's opened, then disable darksend from then on.
- Fixed some bugs with connecting to the correct masternodes
- Send was sending way too many coins for all modes, (I sent 100SUC anon and it
  sent 2000SUC, then sent me change for the rest causing a whole reprocess of
...
- Fees per round of Darksend are 0.001SUC or $0.00538 at current prices. This
  means to anonymize 1000SUC with 3 rounds (an average use case) it would cost
  a user 1.5 cents.
...
- RPC calls are changed a bit:
    ...
    darksend denominate 100 - Will denominate 100SUC
    darksend auto - Will run AutoDenominate
    darksend Xaddr 100 - Will send 100 denominated SUC to Xaddr
    sendtoaddress Xaddr 100 - Will send 100 non-denominated SUC to Xaddr
...
- Added a smart input splitting method. Place 1000+SUC into a brand new wallet
...
- Mixing rounds are limited to 1000SUC, per block
...
- DoAutomaticDenominations 100SUC limit changed to 500SUC (we should see a bunch
