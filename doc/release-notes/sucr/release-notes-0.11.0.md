...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`SUCP`/`SUCV`
  for testnet)
- Adapted BIP44 coin type `5` for Darkcoin (0x80000005) as defined in SLIP-0044
- Added new units: `doufs` (1 / 100.000.000 SUC)
- Added units for testnet: tSUC, mtSUC, utSUC, tdoufs
...
- Reduced lower darksend limit to 1.5 SUC
- Fixed progress bar calculation for low amounts
- Improved support for adding SUC after anon has completed
...
- Add '-disablewallet' mode to let sucrd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with sucrd
...
- Drop the fee required to relay a transaction to 0.01mSUC per kilobyte
